    ...
    detectBlackCells(warpedImage, gridSize) {
        ...
        for (let row = 0; row < gridSize; row++) {
            ...
            for (let col = 0; col < gridSize; col++) {
                ...
                const centerX = Math.floor((col + 0.5) * cellSize);
                const centerY = Math.floor((row + 0.5) * cellSize);
                
                // Sample a small region around center
                const sampleSize = Math.floor(cellSize * 0.3);
                const startX = Math.max(0, centerX - sampleSize);
                const startY = Math.max(0, centerY - sampleSize);
                const endX = Math.min(gray.cols, centerX + sampleSize);
                const endY = Math.min(gray.rows, centerY + sampleSize);
                ...
                // Determine if cell is black (blocked) or white (playable)
                const isBlack = avgIntensity < this.BLOCK_THRESHOLD;
...
class CrosswordGrid {
                ...
                input.addEventListener('input', async (e) => {
                    ...
                    if (HEBREW_RANGE.test(char)) {
                        ...
                        if (next) {
                            ...
                            // Small delay before selecting next
                            setTimeout(() => {
                ...
                input.addEventListener('keydown', async (e) => {
                    if (e.key === 'Backspace') {
                        ...
                        if (prev) {
                            ...
                            // Small delay before selecting previous
                            setTimeout(() => {
    ...
    getWordCells(row, col) {
        ...
        if (this.currentDirection === 'horizontal') {
            // Go right (Hebrew forward: col decreases)
            for (let c = col - 1; c >= 0; c--) {
                if (this.grid[row][c].isBlack) break;
                cells.push({r: row, c: c});
            }
            
            // Go left (Hebrew backward: col increases)
    ...
    getNextCell(row, col) {
        if (this.currentDirection === 'horizontal') {
            // Move left (Hebrew direction: right to left, so col increases)
            for (let c = col + 1; c < this.cols; c++) {
    ...
    getPreviousCell(row, col) {
        if (this.currentDirection === 'horizontal') {
            // Move right (Hebrew backward: left to right, so col decreases)
            for (let c = col - 1; c >= 0; c--) {
                if (!this.grid[row][c].isBlack) {
                    return { row, col: c };
                }
            ...
            // Wrap to previous column (leftward)
            for (let c = col - 1; c >= 0; c--) {
...
class CrosswordApp {
    ...
    async testAzureConnection() {
        ...
        try {
            ...
            // Test with a simple analyze request (text recognition)
            // Using Read API which is what we need for handwriting
            const testUrl = `${cleanEndpoint}/vision/v3.2/read/analyze`;
            
            // Create a minimal test image (white square with black text)
            const canvas = document.createElement('canvas');
            canvas.width = 100;
            canvas.height = 100;
            const ctx = canvas.getContext('2d');
            ...
            if (response.ok || response.status === 202) {
                ...
                if (response.status === 401) {
                    notifier.error('API Key שגוי - בדוק את המפתח');
                } else if (response.status === 404) {
                    notifier.error('Endpoint שגוי - בדוק את הכתובת');
                } else {
                    notifier.error(`שגיאה ${response.status} - בדוק את הפרטים`);
                }
            }
        } catch (error) {
            ...
            if (error.message.includes('Failed to fetch')) {
                notifier.error('בעיית רשת - ודא שה-Endpoint נכון');
            } else {
    ...
    async handleFileSelect(file) {
        ...
        try {
            ...
            if (result.success) {
                ...
                notifier.success(`זוהו ${this.countPlayableCells(result.gridData)} תאים פעילים`);
            } else {
                // Show error
                detectionStatus.querySelector('.status-icon').textContent = '❌';
