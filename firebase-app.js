...
class CrosswordGrid {
    ...
    selectCell(roomId, row, col) {
        ...
        console.log(`Same cell: ${isSameCell}, Time since last: ${timeSinceLastClick}ms`);
        
        // Only toggle direction if:
        // 1. Same cell
        // 2. Within double-click window (500ms)
        // 3. More than 300ms since last click (prevents ANY fast typing from triggering)
        // 4. NOT in a typing sequence (check if last action was typing)
        if (isSameCell && timeSinceLastClick < this.doubleClickDelay && timeSinceLastClick > 300 && !this.isTyping) {
            // Double-click: toggle direction
            console.log('🔄 Double-click detected - toggling direction');
            ...
            // Update direction button display
            const dirBtn = document.getElementById('toggle-direction');
            if (dirBtn) {
                dirBtn.textContent = this.currentDirection === 'horizontal' ? '↔️' : '↕️';
            }
            return;
        }
        
        this.lastClickTime = now;
        this.isTyping = false; // Reset typing flag when cell is clicked
    ...
    async enterRoom(roomId, room) {
        ...
        directionToggle.onclick = () => {
            ...
            directionToggle.textContent = 
                this.crosswordGrid.currentDirection === 'horizontal' ? '↔️' : '↕️';
            notifier.success(
                `כיוון: ${this.crosswordGrid.currentDirection === 'horizontal' ? 'אופקי ↔️' : 'אנכי ↕️'}`
            );
