...
const CONFIG = {
    ...
    GOOGLE_VISION_API_KEY: '', // Set in settings
...
class CrosswordApp {
    ...
    setupEventListeners() {
        ...
        document.getElementById('test-google-vision').addEventListener('click', () => {
            this.testGoogleVisionConnection();
        });
    ...
    async testGoogleVisionConnection() {
        const apiKey = document.getElementById('google-vision-key').value.trim();

        if (!apiKey) {
            notifier.error('מלא את API Key');
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.responses && data.responses[0] && !data.responses[0].error) {
                    CONFIG.GOOGLE_VISION_API_KEY = apiKey;
                    await storageManager.saveData('google_vision_key', apiKey);
                    notifier.success('החיבור תקין! ✅');
                } else {
                    notifier.error('שגיאה - בדוק את המפתח');
                }
            } else {
                const error = await response.json();
                console.error('Google Vision error:', error);
                notifier.error('API Key שגוי או Vision API לא מופעל');
            }
        } catch (error) {
            console.error('Connection test error:', error);
            notifier.error('לא הצלחנו להתחבר ל-Google Cloud');
        }
    }
    ...
    setupTestCanvas() {
        ...
        const draw = (e) => {
            ...
            ctx.lineWidth = 8;  // Thicker for better recognition
            ctx.lineCap = 'round';
            ctx.lineJoin = 'round';
    ...
    async recognizeTestHandwriting() {
        if (!CONFIG.GOOGLE_VISION_API_KEY) {
            notifier.error('הגדר API Key קודם');
            return;
        }
        ...
        try {
            ...
            const response = await fetch(`https://vision.googleapis.com/v1/images:annotate?key=${CONFIG.GOOGLE_VISION_API_KEY}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            if (data.responses && data.responses[0] && data.responses[0].textAnnotations) {
                ...
                console.log('Text annotations:', annotations);
                ...
                if (annotations.length > 0) {
                    ...
                    if (match) {
                        ...
                        console.log('No Hebrew character found in:', text);
                        notifier.error(`זוהה "${text}" - אבל לא אות עברית`);
                        document.getElementById('test-result-text').textContent = text || '???';
                        document.getElementById('test-result').classList.remove('hidden');
                    }
                } else {
                    console.log('No annotations returned');
                    notifier.error('לא זוהה טקסט - נסה לכתוב יותר ברור');
                }
            } else {
                console.log('No textAnnotations in response');
                notifier.error('לא זוהה טקסט - נסה לכתוב יותר ברור');
            }
        } catch (error) {
    ...
    async loadSettingsScreen() {
        ...
        const savedKey = await storageManager.getData('google_vision_key');
        if (savedKey) {
            document.getElementById('google-vision-key').value = savedKey;
            CONFIG.GOOGLE_VISION_API_KEY = savedKey;
        }
