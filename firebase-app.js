...
window.database = database; // Make available globally for shared-drawing.js
...
class ConnectionMonitor {
    constructor() {
        this.isOnline = true;
        this.hasInitialized = false; // NEW: Track if we've completed initial connection
        ...
        connectedRef.on('value', (snapshot) => {
            if (snapshot.val() === true) {
                ...
                if (!this.hasInitialized) {
                    this.hasInitialized = true; // Mark as initialized after first successful connection
                }
            } else {
                // Only show disconnect warning AFTER initial connection established
                if (this.hasInitialized) {
    ...
    handleReconnect() {
        ...
        // Only show "connection restored" message if we were previously offline
        // Don't show on initial page load
        if (!this.wasOffline || !this.hasInitialized) {
...
class RoomManager {
    ...
    async createRoom(crosswordFile, detectedGridData, detectedGridSettings, detectedClues, roomName) {
        try {
            ...
            const room = {
                ...
                detectedClues: detectedClues || null, // Save clue positions from detection
                participants: 1,
                createdAt: Date.now()
            };
...
class CrosswordGrid {
    constructor(containerSelector) {
        ...
        this.isTyping = false; // Track if in typing sequence
    }
    ...
    async initialize(roomId, gridData, gridSettings) {
        ...
        if (gridSettings && gridSettings.position) {
            ...
            let gridWidth = imageRect.width * pos.width / 100;
            let gridHeight = imageRect.height * pos.height / 100;
            
            // Store the aspect ratio from the original positioning
            // This ensures all devices use the EXACT same aspect ratio
            if (!pos.aspectRatio) {
                pos.aspectRatio = pos.width / pos.height;
                console.log(`📏 Stored aspect ratio: ${pos.aspectRatio.toFixed(4)}`);
            }
            
            // Force consistent aspect ratio across devices
            // Use width as primary, adjust height to match stored ratio
            gridHeight = gridWidth / pos.aspectRatio;
            
            console.log(`Grid dimensions: ${gridWidth.toFixed(1)}×${gridHeight.toFixed(1)} (aspect: ${(gridWidth/gridHeight).toFixed(4)})`);
    ...
    createCell(roomId, row, col, cellData) {
        ...
        // Add click listener
        console.log(`📍 Adding click listener to cell [${row}, ${col}]`);
        
        // Add multiple event types to debug
        cell.addEventListener('mousedown', (e) => {
            console.log(`🖱️ MOUSEDOWN on cell [${row}, ${col}]`);
        });
        
        cell.addEventListener('mouseup', (e) => {
            console.log(`🖱️ MOUSEUP on cell [${row}, ${col}]`);
        });
        
        cell.addEventListener('click', (e) => {
            console.log(`🖱️ CLICK EVENT FIRED on cell [${row}, ${col}]`);
            e.stopPropagation();
            this.selectCell(roomId, row, col);
        });
        
        // Check if cell is actually clickable
        cell.addEventListener('pointerenter', () => {
            console.log(`👆 Pointer entered cell [${row}, ${col}]`);
        });
        
        console.log(`✅ Cell [${row}, ${col}] created with click listener`);
        ...
        if (this.isMobile()) {
            ...
            input.setAttribute('aria-label', 'Enter Hebrew letter');
            
            // Prevent zoom on iOS/iPad
            input.style.fontSize = '16px';
            
            // Make input slightly visible on iPad for better interaction
            if (/iPad/.test(navigator.userAgent)) {
                input.style.opacity = '0.01'; // Nearly invisible but interactable
            ...
            input.addEventListener('input', async (e) => {
                ...
                if (HEBREW_RANGE.test(char)) {
                    this.isTyping = true; // Mark as typing to prevent direction change
                    await this.updateCell(roomId, row, col, char);
                    
                    // Clear input first
                    e.target.value = '';
                    
                    // Move to next cell (DON'T change direction)
                    const next = this.getNextCell(row, col);
                    if (next) {
                        // Select next cell without triggering direction change
                        this.selectCellWithoutDirectionChange(roomId, next.row, next.col);
                        
                        // Focus next cell's input
                        setTimeout(() => {
                            const nextCell = this.container.querySelector(`[data-row="${next.row}"][data-col="${next.col}"]`);
            ...
            // Handle backspace and arrow keys (for direction change on mobile)
            input.addEventListener('keydown', async (e) => {
                if (e.key === 'Backspace') {
                    e.preventDefault();
                    await this.updateCell(roomId, row, col, '');
                    
                    // Move to previous cell without changing direction
                    const prev = this.getPreviousCell(row, col);
                    if (prev) {
                        this.selectCellWithoutDirectionChange(roomId, prev.row, prev.col);
                        setTimeout(() => {
                            const prevCell = this.container.querySelector(`[data-row="${prev.row}"][data-col="${prev.col}"]`);
                    ...
                    e.preventDefault();
                    // Change direction by clicking the same cell
                    this.selectCell(roomId, row, col);
                }
            });
    ...
    setupPCKeyboard(roomId) {
        console.log('⌨️ Setting up PC keyboard handler');
        
        // Handle keyboard input on PC (direct typing)
        document.addEventListener('keydown', async (e) => {
            if (this.isMobile()) return;
            if (!this.activeCell) {
                console.log('⚠️ No active cell for keyboard input');
                return;
            }
            ...
            if (e.key === 'Backspace') {
                e.preventDefault();
                console.log('🔙 Backspace - clearing cell');
                await this.updateCell(roomId, row, col, '');
                ...
                e.preventDefault();
                console.log('✍️ Hebrew letter:', e.key);
                ...
                e.preventDefault();
                console.log('➡️ Arrow key:', e.key);
                this.handleArrowKey(e.key, row, col, roomId);
            }
        });
        
        console.log('✅ PC keyboard handler set up');
    }
    ...
    selectCell(roomId, row, col) {
        console.log(`🖱️ Cell clicked: [${row}, ${col}]`);
        
        // REMOVED: Double-click direction toggle completely
        // Direction can ONLY be changed via the button now
        // This prevents any accidental direction changes during typing
    ...
    selectCellWithoutDirectionChange(roomId, row, col) {
        // Select cell but DON'T update lastClickTime (prevents direction toggle)
        // Remove previous active state
        if (this.activeCell) {
            this.activeCell.classList.remove('active');
        }
    ...
    clearWordHighlight() {
        // Cache the highlighted cells for faster clearing
        if (!this.highlightedCells) {
            this.highlightedCells = [];
        }
...
class CrosswordApp {
    ...
    async handleFileSelect(file) {
        ...
        try {
            ...
            if (result.success) {
                ...
                // Run clue detection while we have file access (no CORS issues)
                detectionStatus.querySelector('.status-text').textContent = 'מזהה מספרי רמזים...';
                try {
                    const clueDetector = new ClueDetector();
                    const gridBounds = {
                        left: result.gridSettings.position.right,
                        top: result.gridSettings.position.top,
                        right: result.gridSettings.position.right + result.gridSettings.position.width,
                        bottom: result.gridSettings.position.top + result.gridSettings.position.height
                    };
                    
                    const detectedClues = await clueDetector.detectClues(preview, gridBounds);
                    this.detectedClues = detectedClues; // Store for later use
                    console.log(`✅ Detected ${detectedClues.length} clue numbers during creation`);
                } catch (err) {
                    console.warn('⚠️ Clue detection failed during creation:', err);
                    this.detectedClues = null;
                }
    ...
    async createRoom() {
        ...
        const result = await roomManager.createRoom(
            ...
            this.detectedClues, // Pass detected clue positions
            roomName
        );
    ...
    async enterRoom(roomId, room) {
        ...
        // Don't set crossOrigin - it causes CORS issues with Firebase Storage
        // crosswordImage.crossOrigin = 'anonymous';
        crosswordImage.src = room.crosswordImageUrl;
        
        // Wait for image to load before clue detection
        await new Promise((resolve) => {
            if (crosswordImage.complete) {
                resolve();
            } else {
                crosswordImage.onload = resolve;
            }
        });
        
        console.log('✅ Crossword image loaded');
        ...
        // Initialize shared drawing system (single canvas overlay)
        if (!this.sharedDrawing) {
            this.sharedDrawing = new SharedDrawing();
        }
        this.sharedDrawing.initialize(roomId, this.crosswordGrid.container);
        
        // Initialize clue marking system
        // Use pre-detected clues from room creation (no CORS issues)
        if (!this.clueMarking) {
            this.clueMarking = new ClueMarking();
        }
        
        // Check if we have pre-detected clues
        if (room.detectedClues && room.detectedClues.length > 0) {
            console.log(`📥 Using ${room.detectedClues.length} pre-detected clues`);
            // Initialize with pre-detected clues
            this.clueMarking.initializeWithPreDetectedClues(roomId, room.detectedClues);
        } else {
            // Fallback to manual input
            console.log('📝 No pre-detected clues, using manual input');
            this.clueMarking.initialize(roomId, null, null, false);
        }
        
        // Setup direction toggle button
        const directionToggle = document.getElementById('toggle-direction');
        directionToggle.textContent = '↔️'; // Start horizontal
        directionToggle.title = 'Change direction: Currently Horizontal (↔️)';
        
        directionToggle.onclick = () => {
            this.crosswordGrid.currentDirection = 
                this.crosswordGrid.currentDirection === 'horizontal' ? 'vertical' : 'horizontal';
            
            const isHorizontal = this.crosswordGrid.currentDirection === 'horizontal';
            directionToggle.textContent = isHorizontal ? '↔️' : '↕️';
            directionToggle.title = `Change direction: Currently ${isHorizontal ? 'Horizontal (↔️)' : 'Vertical (↕️)'}`;
            
            notifier.success(
                `כיוון: ${isHorizontal ? 'אופקי ↔️' : 'אנכי ↕️'}`
            );
            
            console.log(`🔄 Direction changed to: ${this.crosswordGrid.currentDirection}`);
            
            // Refresh word highlight
            if (this.crosswordGrid.activeCell) {
                const row = parseInt(this.crosswordGrid.activeCell.dataset.row);
                const col = parseInt(this.crosswordGrid.activeCell.dataset.col);
                this.crosswordGrid.clearWordHighlight();
                this.crosswordGrid.highlightWord(row, col);
            }
        };
        
        // Setup mode toggle (Type vs Draw)
        const modeToggle = document.getElementById('toggle-mode');
        modeToggle.textContent = '⌨️'; // Start in type mode
        modeToggle.onclick = () => {
            const isDrawMode = this.sharedDrawing.toggleDrawMode();
            modeToggle.textContent = isDrawMode ? '🖊️' : '⌨️';
            modeToggle.classList.toggle('active', isDrawMode);
            notifier.success(isDrawMode ? 'מצב ציור 🖊️' : 'מצב הקלדה ⌨️');
            
            // Clear cell selection when entering draw mode
            if (isDrawMode) {
                this.crosswordGrid.clearSelection();
            }
        };
        ...
        undoBtn.onclick = () => {
            this.sharedDrawing.undo();
        };
        
        // Setup redo button
        const redoBtn = document.getElementById('redo-btn');
        redoBtn.onclick = () => {
            this.sharedDrawing.redo();
        };
