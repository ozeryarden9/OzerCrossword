...
class CrosswordApp {
    ...
    async enterRoom(roomId, room) {
        ...
        const recognitionToggle = document.getElementById('toggle-recognition');
        recognitionToggle.classList.add('active'); // Default: enabled
        recognitionToggle.onclick = () => {
            const enabled = this.continuousHandwriting.toggleRecognition();
            recognitionToggle.classList.toggle('active', enabled);
            notifier.success(enabled ? 'זיהוי אוטומטי פעיל 🤖' : 'מצב ציור חופשי 🎨');
        };
        
        // Eraser button
        const eraserBtn = document.getElementById('eraser-btn');
        eraserBtn.onclick = () => {
            this.continuousHandwriting.clearAllDrawings();
            notifier.success('הציורים נמחקו 🧹');
        };
