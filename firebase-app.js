...
class GridDetector {
    ...
    detectBlackCells(warpedImage, gridSize) {
        console.log('🔍 BLACK CELL DETECTION STARTED');
        ...
        // Use adaptive threshold - 10% of pixels must be black
        const BLACK_THRESHOLD = 127; // Pixel values below this are considered black
        const BLACK_PERCENTAGE_THRESHOLD = 0.1; // 10% of cell must be black
        ...
        for (let row = 0; row < gridSize; row++) {
            ...
            for (let col = 0; col < gridSize; col++) {
                ...
                // Count black vs white pixels in inner area only
                ...
                // Cell is black if 25% or more of inner pixels are black
                const isBlack = blackPercentage >= 0.25;
                
                // Debug: log first few cells
                if (row === 0 && col < 3) {
                    console.log(`Cell [${row},${col}]: blackPx=${blackPixelCount}, totalPx=${totalPixelCount}, percentage=${(blackPercentage * 100).toFixed(1)}%, isBlack=${isBlack}`);
                }
