...
class GridDetector {
    ...
    detectBlackCells(warpedImage, gridSize) {
        ...
        // Use adaptive threshold - 30% of pixels must be black
        const BLACK_THRESHOLD = 127; // Pixel values below this are considered black
        const BLACK_PERCENTAGE_THRESHOLD = 0.3; // 30% of cell must be black
    ...
    async handleFileSelect(file) {
        ...
        try {
            ...
            if (result.success) {
                ...
                notifier.success(`זוהו ${121 - blackCount} תאים פעילים ו-${blackCount} תאים חסומים (30% threshold)`);
            } else {
                // Show error
                detectionStatus.querySelector('.status-icon').textContent = '❌';
