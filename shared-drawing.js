    ...
    setupFirebaseSync() {
        ...
        this.strokesListener = strokesRef.on('value', (snapshot) => {
            ...
            if (newStr !== currentStr) {
                // Skip redraw if in a drawing session to prevent interruption
                if (this.isInDrawingSession) {
                    console.log('⏭️ Skipped redraw - in drawing session (preventing interruption)');
                    // Update data but don't redraw yet
                    this.cellStrokes = convertedData;
                    return;
                }
                
                // Check if this is genuinely NEW data (from another device)
                // BEFORE updating local data
                ...
                for (const cellKey in convertedData) {
                    ...
                    // New strokes = remote has MORE strokes than local
                    if (remoteStrokes.length > localStrokes.length) {
                        hasNewStrokes = true;
                        console.log(`📥 Cell ${cellKey} has new strokes: ${localStrokes.length} → ${remoteStrokes.length}`);
                        break;
                    }
                }
                
                // Also check for cells that exist remotely but not locally
                for (const cellKey in convertedData) {
                    if (!this.cellStrokes[cellKey]) {
                        hasNewStrokes = true;
                        console.log(`📥 Cell ${cellKey} is new`);
                        break;
                    }
                }
                ...
                if (hasNewStrokes) {
                    console.log('✅ New strokes from another device - redrawing');
                    this.redrawAllCells();
                } else {
                    console.log('⏭️ No new strokes - skipping redraw (our own update echoed back)');
                }
            } else {
