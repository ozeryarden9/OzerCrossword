...
class SharedDrawing {
    ...
    createCellCanvases() {
        ...
        cells.forEach(cell => {
            ...
            try {
                ...
                canvas.style.boxSizing = 'border-box'; // FIX #3: Prevent padding from affecting size
    ...
    handleCellPointerDown(e, cellKey) {
        ...
        // DON'T cache rect - get it fresh on every point to avoid offset issues
        // this.currentCellRect = canvas.getBoundingClientRect(); // REMOVED
    ...
    handleCellPointerUp() {
        ...
        // No need to clear cached rect since we're not caching anymore
    ...
    getCellPointerPosition(e, cellKey) {
        ...
        // Get both canvas and cell rects
        const canvasRect = canvas.getBoundingClientRect();
        const cellRect = cell.getBoundingClientRect();
        
        // FIX #1: Use targetTouches (only finger on THIS canvas), not touches (all fingers)
        const clientX = e.clientX || (e.targetTouches && e.targetTouches[0] ? e.targetTouches[0].clientX : 0);
        const clientY = e.clientY || (e.targetTouches && e.targetTouches[0] ? e.targetTouches[0].clientY : 0);
        
        // Calculate position relative to canvas (CSS coordinates)
        const pixelX = clientX - canvasRect.left;
        const pixelY = clientY - canvasRect.top;
        
        // FIX #2: Round to integers BEFORE calculating percentage to prevent sub-pixel issues
        const roundedPixelX = Math.round(pixelX);
        const roundedPixelY = Math.round(pixelY);
        
        // Also calculate offset of canvas within cell
        const canvasOffsetX = canvasRect.left - cellRect.left;
        const canvasOffsetY = canvasRect.top - cellRect.top;
        
        // Use INTERNAL canvas dimensions for aspect ratio (accounts for Retina/DPR)
        const internalAspect = canvas.width / canvas.height;
        
        // Store as percentage (0-100) for device independence
        // Round to 1 decimal place to eliminate sub-pixel differences
        const percentage = {
            x: Math.round((roundedPixelX / canvasRect.width) * 1000) / 10,  // Percentage (1 decimal)
            y: Math.round((roundedPixelY / canvasRect.height) * 1000) / 10, // Percentage (1 decimal)
            aspectRatio: internalAspect, // Use internal dimensions for aspect ratio
            t: Date.now()
        };
        
        // Show debug info on screen for ALL devices
        ...
        this.showDebugInfo(
            ...
            `CSS: ${canvasRect.width.toFixed(1)}×${canvasRect.height.toFixed(1)}\n` +
            `Internal: ${canvas.width}×${canvas.height}\n` +
            `Canvas offset: (${canvasOffsetX.toFixed(2)}, ${canvasOffsetY.toFixed(2)})\n` +
            `Relative: (${roundedPixelX}, ${roundedPixelY}) [rounded]\n` +
            `Percent: (${percentage.x.toFixed(1)}%, ${percentage.y.toFixed(1)}%)\n` +
            `Aspect: ${internalAspect.toFixed(3)}`
        );
    ...
    showDebugInfo(text) {
        ...
        let debugDiv = document.getElementById('drawing-debug');
        if (!debugDiv) {
            debugDiv = document.createElement('div');
            debugDiv.id = 'drawing-debug';
    ...
    drawStrokeSegmentInCell(cellKey, p1, p2) {
        ...
        // Get current canvas CSS dimensions for drawing
        const rect = canvas.getBoundingClientRect();
        
        // Convert percentage to pixels directly - NO aspect ratio compensation
        // Since aspects are nearly identical (1.069 vs 1.070), compensation makes it worse
    ...
    redrawCell(cellKey) {
        ...
        console.log(`🎨 REDRAWING cell ${cellKey}`);
        console.trace('Redraw called from:'); // Show call stack
        ...
        console.log(`  - Redrawing ${strokes.length} strokes in cell ${cellKey}`);
