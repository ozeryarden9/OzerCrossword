...
class SharedDrawing {
    ...
    drawStrokeSegmentInCell(cellKey, p1, p2) {
        ...
        // Use CANVAS rect for drawing to ensure perfect iOS-to-iOS sync
        ...
        // Log if drawing near bottom
        if (p1.y > 85 || p2.y > 85) {
            console.log(`🎨 DRAWING BOTTOM - Y: ${p1.y.toFixed(1)}% → ${y1.toFixed(1)}px (canvas height: ${canvasRect.height.toFixed(1)}px)`);
        }
    ...
    setupFirebaseSync() {
        ...
        this.strokesListener = strokesRef.on('value', (snapshot) => {
            ...
            if (needsConversion) {
                ...
                this.isSyncingOwnChanges = true; // Prevent redraw on our own conversion
                strokesRef.set(convertedData)
                    .then(() => {
                        // Reset flag after conversion save completes
            ...
            if (newStr !== currentStr) {
                ...
                if (this.isSyncingOwnChanges) {
                    console.log('⏭️ Skipped redraw - this is our own change (preventing flicker)');
                    // Still update local data, just don't redraw
