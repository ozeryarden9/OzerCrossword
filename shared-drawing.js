...
class SharedDrawing {
    ...
    getCellPointerPosition(e, cellKey) {
        ...
        console.log(`📍 ${isMobile ? 'MOBILE' : 'PC'} - Cell ${cellKey}:`);
        console.log(`  Touch: (${clientX.toFixed(0)}, ${clientY.toFixed(0)})`);
        console.log(`  Canvas rect: left=${canvasRect.left.toFixed(0)}, top=${canvasRect.top.toFixed(0)}, width=${canvasRect.width.toFixed(0)}, height=${canvasRect.height.toFixed(0)}`);
        console.log(`  Relative: (${pixelX.toFixed(1)}, ${pixelY.toFixed(1)})px`);
        console.log(`  Percentage: (${percentage.x.toFixed(1)}%, ${percentage.y.toFixed(1)}%)`);
        console.log(`  Using cached rect: ${!!this.currentCellRect}`);
