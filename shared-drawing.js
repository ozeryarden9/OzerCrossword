...
class SharedDrawing {
    ...
    initialize(roomId) {
        ...
        console.log('🎨 Per-Cell Drawing System initialized');
    ...
    createCellCanvases() {
        ...
        cells.forEach(cell => {
            ...
            canvas.style.pointerEvents = 'none'; // Will be enabled in draw mode
        ...
        console.log(`✅ Created ${cells.length} cell canvases`);
    }
    ...
    drawStrokeSegmentInCell(cellKey, p1, p2) {
        const { ctx } = this.cellCanvases[cellKey];
    ...
    syncStrokesToFirebase() {
        ...
        if (Object.keys(this.cellStrokes).length === 0) {
            ...
            strokesRef.set(this.cellStrokes)
                .then(() => console.log('✅ All cell strokes synced'))
                .catch(err => console.error('❌ Sync error:', err));
        }
    }
