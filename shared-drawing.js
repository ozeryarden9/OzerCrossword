...
class SharedDrawing {
    ...
    getCellPointerPosition(e, cellKey) {
        ...
        const percentage = {
            x: Math.round((pixelX / canvasRect.width) * 1000) / 10,  // Percentage (1 decimal)
            y: Math.round((pixelY / canvasRect.height) * 1000) / 10, // Percentage (1 decimal)
        ...
        this.showDebugInfo(
            ...
            `Relative: (${pixelX.toFixed(1)}, ${pixelY.toFixed(1)})\n` +
    ...
    drawStrokeSegmentInCell(cellKey, p1, p2) {
        ...
        // Get current canvas dimensions (CSS for drawing, internal for aspect)
        const rect = canvas.getBoundingClientRect();
        const currentAspect = canvas.width / canvas.height; // Use internal dimensions
        
        // Get the aspect ratio from when stroke was drawn (also internal)
        const originalAspect = p1.aspectRatio || currentAspect;
        
        // Calculate aspect ratio correction factor
        const aspectCorrection = currentAspect / originalAspect;
        
        // Convert percentage to pixels with aspect ratio compensation
        // X uses direct conversion, Y is adjusted for aspect ratio difference
        const x1 = (p1.x / 100) * rect.width;
        const y1 = (p1.y / 100) * rect.height * aspectCorrection;
        const x2 = (p2.x / 100) * rect.width;
        const y2 = (p2.y / 100) * rect.height * aspectCorrection;
