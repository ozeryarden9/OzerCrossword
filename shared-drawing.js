...
class SharedDrawing {
    ...
    getCellPointerPosition(e, cellKey) {
        ...
        // Also store the canvas size for accurate redrawing
        const percentage = {
            x: (pixelX / canvasRect.width) * 100,  // Percentage
            y: (pixelY / canvasRect.height) * 100, // Percentage
            t: Date.now(),
            // Store canvas dimensions at time of drawing
    ...
    drawStrokeSegmentInCell(cellKey, p1, p2) {
        ...
        // Use the canvas dimensions from when the stroke was CREATED
        // This ensures strokes look the same even if canvas size changes
        const canvasWidth = p1.canvasWidth || canvas.getBoundingClientRect().width;
        const canvasHeight = p1.canvasHeight || canvas.getBoundingClientRect().height;
